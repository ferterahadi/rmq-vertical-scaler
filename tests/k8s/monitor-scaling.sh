...
# Function to get current resources - shows each pod's CPU allocation
get_current_resources() {
    echo "Individual Pod CPU Allocations:"
    kubectl get pods -l app.kubernetes.io/name=rabbitmq -o custom-columns='POD:.metadata.name,CPU:.spec.containers[0].resources.requests.cpu,MEMORY:.spec.containers[0].resources.requests.memory' --no-headers 2>/dev/null | while read line; do
        echo "    $line"
    done
    echo -n "  Cluster Setting: "
